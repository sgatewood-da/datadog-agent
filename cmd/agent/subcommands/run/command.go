...
func run(log log.Component,
	...
	if err := startAgent(cliParams, log, flare, telemetry, sysprobeconfig, server, capture, serverDebug, wmeta, rcclient, logsAgent, forwarder, sharedSerializer, otelcollector); err != nil {
		return err
	}
...
func StartAgentWithDefaults(ctxChan <-chan context.Context) (<-chan error, error) {
	...
	go func() {
		err := fxutil.OneShot(func(
			...
			err := startAgent(&cliParams{GlobalParams: &command.GlobalParams{}}, log, flare, telemetry, sysprobeconfig, server, capture, serverDebug, wmeta, rcclient, logsAgent, forwarder, sharedSerializer, otelcollector)
			if err != nil {
				return err
			}
